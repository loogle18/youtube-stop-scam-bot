...
const checkIfScamComment = (commentText) => {
  ...
  const callToActionPattern = new RegExp('(buy|bought|купи|покупай|упустить|советую|возможность|достало ждать)', 'gi');

  if (
    (trueScamCommentPattern.test(commentText) ||
      tokenPattern.test(commentText)) &&
    !/liquid/gi.test(commentText)
  ) {
