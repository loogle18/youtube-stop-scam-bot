...
const searchPerPage = async function* (publishedAfter) {
  let searchResult = await search(publishedAfter);
  let nextPageToken = searchResult.nextPageToken;
  yield (searchResult ? searchResult.items : []);
  while (searchResult && nextPageToken) {
    searchResult = await search(publishedAfter, nextPageToken);
...
const runProccess = async () => {
  ...
  let searchGenerator =  searchPerPage(publishedAfter);
