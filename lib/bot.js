...
var getCommentText = (authorName) => {
  ...
  text += 'or if you have noticed scam comment without already being reported\n';
  ...
  text += 'или, если вы заметили мошеннический комментарий, о котором еще не сообщили\n';
...
const checkIfScamComment = (commentText) => {
  ...
  const trueScamSitesPattern = new RegExp(`(${scamSites.join('|')}).*(info|site|net|pro|org)`, 'gi');
