...
const checkIfScamComment = (commentText) => {
  const trueScamCommentPattern = /ton-token.*info|tonmarket.*site|ton-gram|sale-gram|gramton.net|mygram.*site.*buygram/gi;
  const callToActionPattern = /buy|bought|купи|покупай|упустить|советую|возможность|достало ждать/gi;

  if (trueScamCommentPattern.test(commentText) && callToActionPattern.test(commentText)) {
